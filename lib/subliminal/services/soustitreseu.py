...
class Soustitreseu(ServiceBase):
    server_url = 'http://www.sous-titres.eu/series'
    site_url = 'http://www.sous-titres.eu/'
    api_based = False
    ...
    def query(self, filepath, languages, keywords=None, series=None, season=None, episode=None):
        ...
        if series and season and episode:
            ...
            r = self.session.get('%s/%s.html' % (self.server_url, urllib.quote(request_series)))
        ...
        subtitles = []
        subtitles_list = soup.find('div', {'id' : 'saison-%s' % season})
        if subtitles_list :
            link_list = subtitles_list.findAll('a', {'class' : 'subList'})
                ...
                for episode_naming in possible_episode_naming :
                    if episode_naming in filename :
                        for language in languages:
                            ...
                            subtitle = ResultSubtitle(path, language, self.__class__.__name__.lower(), '%s/%s' % (self.server_url,link_url))
